...
export async function startVoice(callbacks) {
  ...
  try {
    ...
    return {stop,resumeAudio:async()=>{try{await audioContext.resume();await audio.play();callbacks.blocked(false);}catch{callbacks.notice('Audio is blocked. Check the browser’s audio permissions.');}}};
  } catch(error) {
